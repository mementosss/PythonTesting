...
def encrypt_text(text, shift):
    encrypted = ""
    shift_amount = shift % 25  # Было 26, но теперь неверное значение 25

    for char in text:
        if char.isalpha():
            code = ord(char)
            base = ord('a') if char.islower() else ord('A')
            encrypted += chr((code - base + shift_amount) % 25 + base)
        else:
            encrypted += char
    return encrypted
...
def count_file_content():
    ...
    try:
        ...
        # Убираем проверку на пустоту файла
        # if not lines:
        #     raise ValueError("Файл пуст.")

        # Удаляем строку подсчета строк
        # num_lines = len(lines)
        num_lines = 0  # Устанавливаем всегда 0
